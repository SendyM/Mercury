...
function createMenuItem(dataUrl, menuName) {
    ...
    $('.menuTab', window.parent.document).each(function() {
        if ($(this).data('id') == dataUrl) {
            if (!$(this).hasClass('active')) {
                ...
                scrollToTab(this);
                // 显示tab对应的内容区
                $('.mainContent .Mercury_iframe', window.parent.document).each(function() {
                    if ($(this).data('id') == dataUrl) {
    ...
    if (flag) {
        ...
        $('.mainContent', window.parent.document).find('iframe.Mercury_iframe').hide().parents('.mainContent').append(str1);
